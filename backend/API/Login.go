...
func LoginNew(c *gin.Context) {
	...
	chkExp, err1 := http.Get("http://192.168.161.102/api_system/getAccountEx?username=" + strings.ToUpper(objUserCheck.Su_username))
	if err1 != nil {
		c.IndentedJSON(http.StatusInternalServerError, gin.H{"error": "Failed to fetch data from external API"})
		return
	}
	...
	hasher := md5.New()
	hasher.Write([]byte(objUserCheck.Su_password))
	md5Hash := hex.EncodeToString(hasher.Sum(nil))

	if md5Hash != accountsExp[0].PASSWORD {
		c.IndentedJSON(http.StatusOK, "Password incorrect")
		return
	} else {
		var objUser Users

		chkLoginExp := db.QueryRow("SELECT * FROM sys_users WHERE su_username = ?", strings.ToUpper(objUserCheck.Su_username)).Scan(&objUser.Su_id, &objUser.Spg_id, &objUser.Su_username, &objUser.Su_password, &objUser.Su_firstname, &objUser.Su_lastname, &objUser.Su_email, &objUser.Sd_id, &objUser.Su_sign_path, &objUser.Su_sign_file, &objUser.Su_status, &strCreateDate, &strCreateBy, &strUpdateDate, &strUpdateBy, &objUser.Su_last_accress)
		...
		_, err := db.Exec("UPDATE sys_users SET su_password = ? WHERE su_username = ?", md5Hash, strings.ToUpper(objUserCheck.Su_username))
		if err != nil {
			c.IndentedJSON(http.StatusOK, gin.H{"Error": err.Error()})
			return
		}

		chkLogin := db.QueryRow("SELECT * FROM sys_users WHERE su_username = ? AND su_password = ?", strings.ToUpper(objUserCheck.Su_username), md5Hash).Scan(&objUser.Su_id, &objUser.Spg_id, &objUser.Su_username, &objUser.Su_password, &objUser.Su_firstname, &objUser.Su_lastname, &objUser.Su_email, &strDept, &strUserImgPath, &strUserImgName, &objUser.Su_status, &strCreateDate, &strCreateBy, &strUpdateDate, &strUpdateBy, &objUser.Su_last_accress)
		...
		if chkLogin == sql.ErrNoRows {
			c.IndentedJSON(http.StatusOK, "User not found")
			return
		} else {
			if objUser.Su_status == 0 {
				c.IndentedJSON(http.StatusOK, "This user is banned")
				return
			}
