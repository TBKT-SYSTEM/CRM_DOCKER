                    ...
                    <div class="card-body">
                        ...
                        <div class="row border" style="padding: 15px;">
                            <div class="table-responsive">
                                <table id="tblMngApproval" class="dataTable table  table-bordered text-nowrap align-middle" style="width: 100%;">
                                    <thead>
                                        <!-- start row -->
                                        <tr>
...
<script>
    ...
    async function ViewAll() {
        if ($.fn.DataTable.isDataTable('#tblMngApproval')) {
            $('#tblMngApproval').DataTable().destroy();
        }
        const currentYear = new Date().getFullYear();
        const firstDayOfYear = `${currentYear}-01-01`;
        const lastDayOfYear = `${currentYear}-12-31`;
        const su_id = '<?php echo $this->session->userdata('sessUsrId') ?>';
        dataTable = $('#tblMngApproval').DataTable({
            ajax: {
                url: API_URL + 'manageApproval/viewAll/' + firstDayOfYear + '/' + lastDayOfYear + '/' + su_id,
            },
            columnDefs: [{
                searchable: true,
                orderable: true,
    ...
    function searchDate() {
        ...
        dataTable = $('#tblMngApproval').DataTable({
            ajax: {
                url: API_URL + 'manageApproval/searchDate' + '/' + startDate + '/' + endDate + '/' + su_id,
    ...
    $(document).ready(function() {
        ...
        dataTable = $('#tblMngApproval').DataTable({
            ajax: {
                url: API_URL + 'manageApproval/table/' + su_id,
                dataSrc: function(json) {
                    return json.data ? json.data : [];
                },
            },
